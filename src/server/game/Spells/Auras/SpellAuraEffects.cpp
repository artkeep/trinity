    ...
    switch(GetAuraType())
    {
        ...
        case SPELL_AURA_DUMMY:
            ...
            if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellProto->SpellFamilyFlags[1] & 0x400)
                amount = caster->SpellHealingBonus(GetBase()->GetUnitOwner(), SpellEffIndex(GetEffIndex()), GetSpellProto(), amount, SPELL_DIRECT_DAMAGE);
            break;
...
void AuraEffect::PeriodicTick(AuraApplication * aurApp, Unit * caster) const
{
    ...
    switch(GetAuraType())
    {
        ...
        case SPELL_AURA_PERIODIC_LEECH:
        {
            ...
            uint32 heal = uint32(caster->SpellHealingBonus(caster, SpellEffIndex(GetEffIndex()), GetSpellProto(), uint32(new_damage * gainMultiplier), DOT, GetBase()->GetStackAmount()));
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            else
            {
                ...
                damage = caster->SpellHealingBonus(target, SpellEffIndex(GetEffIndex()), GetSpellProto(), damage, DOT, GetBase()->GetStackAmount());
            }
...
void AuraEffect::HandleModPowerRegenPCT(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetMiscValue() == POWER_MANA)
        ...
        target->ToPlayer()->UpdateRuneRegen(GetMiscValueB());
    // other powers are not immediate effects - implemented in Player::Regenerate, Creature::Regenerate
}

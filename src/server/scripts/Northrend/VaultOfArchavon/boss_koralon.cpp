...
public:
    ...
    struct boss_koralonAI : public ScriptedAI
    {
		...
		void WGstate(Unit* target)
		{
			if(OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr->GetOutdoorPvPToZoneId(4197))
			{
			 if ((pvpWG->getDefenderTeam()==TEAM_ALLIANCE) && (target->ToPlayer()->GetTeam() == ALLIANCE))
			 {
				return;
			 }
			 else if ((pvpWG->getDefenderTeam()!=TEAM_ALLIANCE) && (target->ToPlayer()->GetTeam() == HORDE))
			 {
				return;
			 }
			 else 
			 {
                 me->MonsterSay("BUGGERS DETECTED !!!", LANG_UNIVERSAL, NULL);
                 target->CastSpell(target, SPELL_TELEPORT_FORTRESS, true); 
                 me->CombatStop(true); 
                 Reset(); 
             }
			}

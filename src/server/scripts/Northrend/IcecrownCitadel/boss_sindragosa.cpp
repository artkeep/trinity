    ...
    public:
        ...
        struct boss_sindragosaAI : public BossAI
        {
            ...
            void Cleanup()
            {
                ...
                instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_FROST_BREATH);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_UNCHAINED_MAGIC:
                        {
                            ...
                            uint8 maxTargets = std::min((uint8)urand(2, 4), (uint8)manaUsers.size());
                        ...
                        //    {
                        //        frostBombLandingMark->CastSpell(frostBombLandingMark, SPELL_FROST_BOMB_EXPLOSION, false);
            ...
            void UpdateAI(const uint32 uiDiff)
            {
                ...
                if (events.ExecuteEvent() == EVENT_ASPHYXIATION)
                    if (Player* IceTomb = ObjectAccessor::GetPlayer(*me, uiTargetGUID))
                        ...
                        IceTomb->CastSpell(IceTomb, SPELL_ASPHYXATION, true);
            }
...
{
    public:
        spell_sindragosa_frost_beacon() : SpellScriptLoader("spell_sindragosa_frost_beacon") { } //70126 Frost Beacon
        ...
        class spell_sindragosa_frost_beacon_AuraScript : public AuraScript
        {
            ...
            void Register()
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_sindragosa_frost_beacon_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
            }
        };
    ...
    public:
        ...
        class spell_sindragosa_ice_tomb_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                if (GetHitUnit()->GetTypeId() == TYPEID_PLAYER)
                {
                    Creature* tomb = GetCaster()->SummonCreature(CREATURE_ICE_TOMB, GetHitUnit()->GetPositionX(), GetHitUnit()->GetPositionY(), GetHitUnit()->GetPositionZ(), 0, TEMPSUMMON_CORPSE_DESPAWN);
                    if(tomb)
                    {
                        tomb->CastSpell(tomb, SPELL_WITHOUT_ANIMATION, true);
                        tomb->SetReactState(REACT_PASSIVE);
                        tomb->AI()->SetGUID(GetHitUnit()->GetGUID());
                    }
                    GetHitUnit()->AddUnitState(UNIT_STAT_STUNNED);
                    GetHitUnit()->SetFlag(UNIT_FIELD_FLAGS,UNIT_FLAG_STUNNED); 
                }
            }

            void Register()
            {
                OnEffect += SpellEffectFn(spell_sindragosa_ice_tomb_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
            }
        };
        ...
        struct npc_icc_frost_bomb_landing_markAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 uiDiff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    {
                        case EVENT_FROST_BOMB_EXPLOSION:
                            me->CastSpell(me, SPELL_FROST_BOMB_EXPLOSION, false);
...
class spell_sindragosa_frost_bomb_explosion : public SpellScriptLoader
{
public:
	spell_sindragosa_frost_bomb_explosion() : SpellScriptLoader("spell_sindragosa_frost_bomb_explosion") { }

	class spell_sindragosa_frost_bomb_explosion_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sindragosa_frost_bomb_explosion_SpellScript);
		...
		void PreventHitByLoS()
		{
			if (Unit* target = GetHitUnit())
			{
				...
				std::list<Creature*> blockList;
                caster->GetCreatureListWithEntryInGrid(blockList, CREATURE_ICE_TOMB, 100.0f);
				if (!blockList.empty())
				{
					for (std::list<Creature*>::const_iterator itr = blockList.begin(); itr != blockList.end(); ++itr)
					{
						if ((*itr)->IsInBetween(caster, target, 4.0f))
						{
                            m_prevented[target->GetGUID()] = true;
		...
		void Register()
		{
			BeforeHit += SpellHitFn(spell_sindragosa_frost_bomb_explosion_SpellScript::PreventHitByLoS);
			AfterHit += SpellHitFn(spell_sindragosa_frost_bomb_explosion_SpellScript::RestoreImmunity);
		}
    private:
		std::map<uint64, bool> m_prevented;
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sindragosa_frost_bomb_explosion_SpellScript();
	}
}; 
...
void AddSC_boss_sindragosa()
{
    ...
    new spell_sindragosa_frost_bomb_explosion();
    new spell_icy_blast();

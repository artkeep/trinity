    ...
    switch(dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            if (dummySpell->SpellIconID == 2236)
            {
                ...
                {
                    if (AuraEffect const * aurEff = pVictim->GetAuraEffect(SPELL_AURA_PERIODIC_DUMMY, SPELLFAMILY_HUNTER, 0x0, 0x80000000, 0x0, GetGUID()))
                        basepoints0 = CalculatePowerCost(aurEff->GetSpellProto(), this, SpellSchoolMask(aurEff->GetSpellProto()->SchoolMask)) * 4 / 10 / 3;
                }
                else
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 12867:
        {
            if (procFlags & PROC_FLAG_DONE_OFFHAND_ATTACK)
                basepoints0 = int32(OFF_ATTACK) + 1;
            else
                basepoints0 = int32(BASE_ATTACK) + 1;
...
{
    if (!spellProto || (spellProto->AttributesEx6 & SPELL_ATTR6_UNK29) || !pVictim || damagetype == DIRECT_DAMAGE)
        return pdamage;
...
uint32 Unit::SpellHealingBonus(Unit * pVictim, SpellEffIndex effIndex, SpellEntry const * spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    // For totems get healing bonus from owner (statue isn't totem in fact)
    if (GetTypeId() == TYPEID_UNIT && this->ToCreature()->isTotem())
        if (Unit* owner = GetOwner())
            return owner->SpellHealingBonus(pVictim, effIndex, spellProto, healamount, damagetype, stack);
    ...
    if (DoneAdvertisedBenefit || TakenAdvertisedBenefit)
    {
        ...
        if (Player * modOwner = GetSpellModOwner())
        {
            ...
            if (effIndex < MAX_SPELL_EFFECTS)
                DoneAdvertisedBenefit = modOwner->ApplyEffectModifiers(spellProto, effIndex, DoneAdvertisedBenefit);
        }
...
bool Unit::HandleStatModifier(UnitMods unitMod, UnitModifierType modifierType, float amount, bool apply)
{
    ...
    switch (modifierType)
    {
        ...
        case TOTAL_PCT:
            ...
            {
                // Dual wield penalty is multiplicative
                m_auraModifiersGroup[unitMod][modifierType] /= 0.5f;
                m_auraModifiersGroup[unitMod][modifierType] += (apply ? amount : -amount) / 100.0f;
                m_auraModifiersGroup[unitMod][modifierType] *= 0.5f;
            }
            break;
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit * pTarget, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellEntry const * procSpell, uint32 damage, SpellEntry const * procAura)
{
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
		...
		if (procSpell && !active)
            active = !isVictim && triggerData.aura->HasEffectType(SPELL_AURA_MOD_POWER_COST_SCHOOL) && (procSpell->SpellFamilyName == SPELLFAMILY_ROGUE) && procSpell->manaCost;
        if (isVictim)

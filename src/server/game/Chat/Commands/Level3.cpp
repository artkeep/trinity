...
bool ChatHandler::HandleBanCharacterCommand(const char *args)
{
   ...
   char* reason = strtok(NULL,"");
    ...
    switch (sWorld->BanCharacter(name, duration, reason, m_session ? m_session->GetPlayerName() : ""))
    {
        case BAN_SUCCESS:
        {
            // РђРІС‚Рѕ Р°РЅРѕРЅСЃ Рѕ Р±Р°РЅРµ РїРµСЂСЃРѕРЅР°Р¶Р°
            if (atoi(duration)>0)
                HandleBanCharMessege(name.c_str(),secsToTimeString(TimeStringToSecs(duration),true).c_str(),m_session->GetPlayerName(),reason);
            else
                HandleBanCharMessege(name.c_str(),GetTrinityString(LANG_PERMANENT_MESSAGE),m_session->GetPlayerName(),reason);
            break;
        }
    }
